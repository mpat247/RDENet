...
from CNN import CNN  # Ensure CNN.py is in the same directory or properly referenced
...
class FashionMNIST_CNNClassifier:
    def __init__(self, batch_size=32, lr=1e-3, epochs=50, device=None, save_path='./outputs/cnn_with_resnet'):
    ...
    def initialize_model(self):
        ...
        self.logger.info("Initializing CNN model.")
        print("Initializing CNN model.")
        model = CNN(**self.params).to(self.device)
        self.logger.info("CNN model initialized successfully.")
    ...
    # --- Testing Methods ---
    def test_model(self, test_loader, test_type='test', training_type='original'):
        """
...
def get_next_run_folder(outputs_dir='./outputs', base_name='cnn_with_resnet'):
    """
...
if __name__ == "__main__":
    ...
    run_folder, is_resumed = get_next_run_folder(outputs_dir='./outputs', base_name='cnn_with_resnet')

    # Initialize classifier with the run folder as save_path
    classifier = FashionMNIST_CNNClassifier(batch_size=64, lr=1e-3, epochs=40, save_path=run_folder)
